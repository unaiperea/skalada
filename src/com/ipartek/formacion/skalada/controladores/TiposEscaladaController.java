 ...
 */
public class TiposEscaladaController extends HttpServlet {
	...
	private void listar(HttpServletRequest request, HttpServletResponse response) {
		...
		request.setAttribute("tipoEscalada", modelo.getAll() );
		
		dispatcher = request.getRequestDispatcher( Constantes.VIEW_BACK_TIPOESCALADA_INDEX );
	...
	private void nuevo(HttpServletRequest request, HttpServletResponse response) {
		...
		tipoEscalada = new TipoEscalada("nuevo"); 
		request.setAttribute("tipoEscalada", tipoEscalada );
		request.setAttribute("titulo", "Crear nuevo registro" );
		
		dispatcher = request.getRequestDispatcher( Constantes.VIEW_BACK_TIPOESCALADA_FORM);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if (tipoEscalada.getId() == -1){
				modelo.save(tipoEscalada); //guardamos el nuevo reg
				request.setAttribute("msg", "Registro creado con éxito"); //Enviamos un mensaje al cliente en el atributo msg
			}else{
				modelo.update(tipoEscalada); //hacemos update del existente
