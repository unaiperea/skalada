...
public class SignupController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			usuarioParaValidar = modeloUsuario
					.getByEmail(pEmailParaValidar);
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (!modeloUsuario.checkUser(pNombre, pEmail)) {
				...
				rol = modeloRol.getById(pIdRol);
				usuario = new Usuario(pNombre, pEmail,
						pPass, rol);
				// Guardar en bbdd
	...
	private boolean enviarEmail() {
		...
		try {
			...
			/*
			 * O DE ESTA FORMA leyendo el fichero directamente desde una ubicación dentro de Webcontent archivo = new File
			 * (Constantes.EMAIL_TEMPLATE_REGISTRO); cuerpo = FileUtils.readFileToString(archivo, "UTF-8"); cuerpo = cuerpo.replace("{usuario}",
			 * usuario); //Los {} pueden ser $ &, cualquier símbolo cuerpo = cuerpo.replace("{url}", url);
			 */
